"""PneumoniaMNIST parser for chest X-ray metadata and labels."""
...
DISEASE_CANONICAL_NAME = "pneumonia"
DATASET_CANONICAL_NAME = "PneumoniaMNIST"
...
def _read_npz_like_payload(path: Path) -> dict:
    try:
        ...
        raise RuntimeError("numpy is required to parse PneumoniaMNIST npz files.") from exc
...
def _add_edge(
    ...
    *,
    ...
    knowledge_layer: str,
    evidence_id: str,
    extraction_method: str = "pneumoniamnist_parser",
    confidence: str = "1.0",
    raw_relation: str = "",
...
class PneumoniaMNISTParseOutputs:
    nodes: List[dict]
    edges: List[dict]
    images: List[dict]
    stats: dict
...
def parse_pneumoniamnist_records(payload: dict, source_id: str) -> PneumoniaMNISTParseOutputs:
    nodes: dict[str, dict] = {}
    edges: dict[str, dict] = {}
    ...
    dataset_id = _add_node(
        ...
        knowledge_layer="C",
        extras={"description": "MedMNIST PneumoniaMNIST"},
    )
    ...
    disease_id = _add_node(
        ...
        knowledge_layer="C",
        extras={"description": "Respiratory infection condition captured in Chest X-rays"},
    )
    ...
    return PneumoniaMNISTParseOutputs(
...
def parse_pneumoniamnist(
    repo_root: Path,
    *,
    source_id: str | None = None,
    manifest_path: Path | None = None,
) -> PneumoniaMNISTParseOutputs:
    manifest = _read_manifest(manifest_path or (repo_root / "data" / "source_manifest.yaml"))
    selected_source = source_id or "pneumoniamnist"
    ...
    if not source_path.exists():
        raise FileNotFoundError(f"PneumoniaMNIST source file missing: {source_path}")

    payload = _read_npz_like_payload(source_path)
    return parse_pneumoniamnist_records(payload, selected_source)


def export_pneumoniamnist_interim_outputs(parsed: PneumoniaMNISTParseOutputs, output_dir: Path) -> dict[str, Path]:
    output_dir.mkdir(parents=True, exist_ok=True)
    out_nodes = output_dir / "pneumoniamnist_nodes.csv"
    out_edges = output_dir / "pneumoniamnist_edges.csv"
    out_images = output_dir / "pneumoniamnist_images.csv"
